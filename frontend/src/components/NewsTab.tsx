  ...
  return (
    <div className="flex flex-col gap-3 overflow-y-auto pr-1 h-full">
      {newsStore.items.map((item) => (
        ...
        >
          <div className="text-sm font-medium text-zinc-200 pb-3 leading-snug">{item.title}</div>
          {item.summary && item.summary !== item.title && (
            <div className="text-xs text-zinc-400 line-clamp-2 mb-2">{item.summary}</div>
          )}
